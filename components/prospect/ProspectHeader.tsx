        ...
        <div className="flex flex-col md:flex-row justify-between md:items-start gap-6">
            <div className="flex-1">
                ...
                <div className="mt-6 relative inline-block">
                    <button 
                        ...
                        className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-900 px-4 py-2 rounded-full text-sm font-semibold transition-colors border border-slate-200"
                    >
                        <span className={`w-2 h-2 rounded-full ${prospect.status === ProspectStatus.Interesting ? 'bg-brand-500' : 'bg-slate-400'}`}></span>
            ...
            <div className="text-right">
                ...
                <button 
                    ...
                    className="mt-4 text-sm text-brand-600 hover:text-brand-800 font-medium underline underline-offset-4"
                >
                    {hasPriceHistory ? t('prospect.updatePrice') : t('prospect.addPrice')}
                </button>
                
                {isAddingPrice && (
                    <form onSubmit={handleAddPriceSubmit} className="mt-3 bg-slate-50 p-3 rounded-lg border border-slate-200 text-left animate-in fade-in slide-in-from-top-2 absolute right-0 z-20 shadow-lg min-w-[240px]">
                        
                        {/* Date Selection */}
                        <div className="mb-2">
                             <label className="text-xs text-slate-500 font-semibold uppercase">{t('prospect.priceForm.date')}</label>
                             <input
                                type="date"
                                value={priceDate}
                                onChange={e => setPriceDate(e.target.value)}
                                className="w-full mt-1 px-2 py-1.5 border border-slate-300 rounded text-sm bg-white text-slate-900 focus:border-brand-500 outline-none"
                            />
                        </div>

                        <div className="mb-3">
                             <label className="text-xs text-slate-500 font-semibold uppercase">{t('prospect.priceForm.newPrice')}</label>
                             <input
                                type="number"
                                value={newPrice}
                        ...
                        <div className="flex justify-end gap-2">
                             <button type="button" onClick={() => setIsAddingPrice(false)} className="text-xs text-slate-500 px-2 py-1">{t('common.cancel')}</button>
                             <button type="submit" className="bg-slate-900 text-white px-3 py-1.5 rounded text-xs font-medium">
