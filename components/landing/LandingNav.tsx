  ...
  return (
    <nav className="flex justify-between items-center p-6 md:px-12 max-w-7xl mx-auto w-full relative z-50">
      ...
      <div className="flex items-center gap-4">
         ...
         <button onClick={login} className="text-slate-600 hover:text-slate-900 font-medium px-4 py-2 hidden sm:block">{t('landing.nav.signIn')}</button>
         <button onClick={login} className="bg-slate-900 text-white px-5 py-2 rounded-lg font-medium hover:bg-slate-800 transition-colors">{t('landing.nav.getStarted')}</button>
         <button onClick={devLogin} className="bg-red-500 text-white px-5 py-2 rounded-lg font-medium hover:bg-red-600 transition-colors ml-2">Test Login</button>
      </div>
    </nav>
  );
};
